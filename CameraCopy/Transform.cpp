...
void Transform::MoveRelative(float x, float y, float z)
{
	...
	DirectX::XMVECTOR rotationQuat = DirectX::XMQuaternionRotationRollPitchYawFromVector(DirectX::XMLoadFloat3(&f3Rotation));
	
	DirectX::XMVECTOR dir = DirectX::XMVector3Rotate(moveVector, rotationQuat);
	DirectX::XMStoreFloat3(&f3Position, DirectX::XMLoadFloat3(&f3Position));
	updateMatrices = true;
}
...
void Transform::UpdateAxis()
{
	...
	XMStoreFloat3(&up, DirectX::XMVector3Rotate(DirectX::XMVectorSet(0, 1, 0, 0), rotQuat));
	XMStoreFloat3(&right, DirectX::XMVector3Rotate(DirectX::XMVectorSet(1, 0, 0, 0), rotQuat));
	XMStoreFloat3(&forward, DirectX::XMVector3Rotate(DirectX::XMVectorSet(0, 0, 1, 0), rotQuat));
	updateVectors = false;
}