...
Mesh::Mesh(Vertex vertices[],int vertexCount,unsigned int indices[], int indexCount) {
	...
	CreateBuffers(vertices, vertexCount, indices, indicesCount);
}
...
void Mesh::CreateBuffers(Vertex* vertArray, size_t numVerts, unsigned int* indexArray, size_t numIndices)
{
...

	{
		...
		initialVertexData.pSysMem = vertArray; // pSysMem = Pointer to System Memory
	...
	//Index buffer
	{
		...
		initialBufferData.pSysMem = indexArray; // pSysMem = Pointer to System Memory
